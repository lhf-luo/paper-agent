# ==============================================================================
# Paper Agent 鈥?Windows 鍚姩鍣?
#
# 銆愮郴缁熶緷璧?棣栨浣跨敤鍓嶉渶瀹夎)銆?
#   1. Node.js 24.x  (瀹樻柟瀹夎鍖? https://nodejs.org)   鈥斺€?蹇呴渶, 浣庝簬 24 鏃犳硶鐩存帴鎵ц TypeScript
#        winget install OpenJS.NodeJS.LTS
#   2. Python 3.10+  (涓嬭浇 arXiv/PDF 闇€瑕? 宸叉湁鍒欒 PAPER_AGENT_PYTHON_BIN 鎸囧悜 python.exe)
#        妫€鏌? python --version
#   3. Poppler      (pdftotext / pdfinfo / pdftoppm, 璇诲彇 PDF 蹇呴渶)
#        winget install oschwartz.116.11  (鎴?choco install poppler)
#        瑁呭畬鍚庢妸 Poppler 鐨?bin 鐩綍鍔犲叆绯荤粺 PATH
#   4. Tesseract OCR (鍥剧墖琛ㄦ牸/鎵嬪啓 OCR, 鍙€?
#        winget install UB-Mannheim.TesseractOCR
#        骞跺畨瑁呬腑鏂囪瑷€鍖?chi_sim (瀹夎鍣ㄥ嬀閫?
#
# 銆愰」鐩?npm 渚濊禆銆?鐢辨湰鑴氭湰鑷姩鎵ц `npm ci` 瀹夎(鏃犻渶鎵嬪姩):
#   @earendil-works/pi-coding-agent, typebox, react, react-dom, react-markdown,
#   remark-gfm, vite, undici 绛?鈥斺€?鍏ㄩ儴鏉ヨ嚜 package.json / package-lock.json
#
# First-use setup: run `paper-agent init`, then `paper-agent models add`.
# ==============================================================================
[CmdletBinding()]
param(
...
Start and paper workflows:
  ...
  paper-agent --agent paper.pdf "Focus on the method and evidence"
...
function Invoke-Install {
	...
	Install-CommandShim
	$configPath = Join-Path $ProjectRoot ".paper-agent\config"
	if (-not (Test-Path -LiteralPath $configPath -PathType Container)) {
		Write-Host ""
		Write-Host 'Split config directory not found. First-use setup:' -ForegroundColor Yellow
		Write-Host "  1) Run: paper-agent init" -ForegroundColor Cyan
		Write-Host "  2) Add a model key: paper-agent models add" -ForegroundColor Cyan
	}
	Invoke-NodeScript -Script "scripts\initialize-storage.ts"
...
function Invoke-Models {
	param([string[]]$ModelArguments = @())
	Invoke-NodeScript -Script "scripts\configure-model.ts" -ScriptArguments $ModelArguments
}
...
function Invoke-Agent {
	...
	& (Join-Path $ScriptsRoot "start-local.ps1") -PdfPath $PdfPath -Instructions $Instructions
	exit $LASTEXITCODE
}
...
function Invoke-OptionSyntax {
	...
	while ($index -lt $Tokens.Count) {
		...
		if ($useAgent) {
			Invoke-Agent -PdfPath $pdfPath -Instructions $instructions
		} else {
...
switch ($normalizedAction) {
	...
	"init" { Invoke-Init }
	"models" { Invoke-Models -ModelArguments $remaining }
	"agent" {
