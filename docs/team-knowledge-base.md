...
**Categories are requested, then applied on approval.** Only a `reviewer` may write **Topic collections** (categories), so a proposal cannot file itself: a contributor may ask for existing categories by passing their ids, and the request is recorded on the review envelope. Approving the paper applies the request — the same decision that publishes the record also vouches for its membership — and the response reports which categories were applied and which were skipped because they no longer exist. A request naming an unknown category is refused before the proposal is written, and a request for an **already published** paper is refused outright, because a published record has no review step left and accepting it would be an unreviewed category write.
...
Readers can use the Web team page to search shared paper metadata by free text and a publication-year range. The browser requests bounded pages and follows the opaque cursor returned by the service; it does not download an unrestricted namespace snapshot. The authenticated HTTP service also supports author, venue, publication-type, open-access, and **topic** filters for client/tool integrations. Scoping a search to one or more topics returns the union of their paper references, so shared categories act as a filter on the paper list rather than a separate browsing mode; an unknown topic id yields an empty page instead of an error.
