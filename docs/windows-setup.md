...
| Python 3.12 | `<PROJECT>\dependence\python\` | arXiv/PDF 下载通道 | ✅ 必需 |
...
## 第 3 步：Python 3.12（装到 dependence\python）

```powershell
# 1) 下载 embeddable 包:
#    https://www.python.org/ftp/python/3.12.10/python-3.12.10-embed-amd64.zip
# 2) 解压到 dependence\python
Expand-Archive python-3.12.10-embed-amd64.zip -DestinationPath dependence\python

# 3) 设置环境变量（下载通道用它; 不设则回退 PATH 中的 python3）:
[Environment]::SetEnvironmentVariable("PAPER_AGENT_PYTHON_BIN", "<PROJECT>\dependence\python\python.exe", "User")

# 4) 验证:
dependence\python\python.exe -c "import sys; print(sys.version)"
```
...
- **python 下载失败**：确认 `PAPER_AGENT_PYTHON_BIN` 指向 dependence\python\python.exe。
