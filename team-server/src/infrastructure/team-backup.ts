...
export async function createTeamBackupBundle(input: {
	...
	if (inside(namespaceRoot, destinationRoot))
		throw new Error("Backup destination must not be inside the namespace root");
...
export async function validateTeamBackupBundle(backupPath: string): Promise<TeamBackupManifest> {
	...
	const registry = JSON.parse(await readFile(join(root, "_security", "identities.json"), "utf8")) as {
		...
		identities?: Array<{ id?: string; tokenSha256?: string; namespaces?: unknown }>;
	};
	if (
		registry.schemaVersion !== 2 ||
		!Array.isArray(registry.identities) ||
		!registry.identities.every(
			(identity) =>
				/^u-[A-Za-z0-9_-]{8,64}$/.test(identity.id ?? "") &&
				/^[a-f0-9]{64}$/i.test(identity.tokenSha256 ?? "") &&
				Array.isArray(identity.namespaces),
		)
	) {
...
export async function runTeamBackupRestoreDrill(
	...
	try {
		...
		const recordCount = (await readdir(join(restoredRoot, "records")).catch(() => [])).filter((name) =>
			name.endsWith(".json"),
		).length;
		...
		return {
			...
			stats: {
				recordCount,
				derivedCount: manifest.files.filter((file) =>
					/^namespace\/knowledge\/derived\/[^/]+\.json$/.test(file.path),
				).length,
				artifactCount: manifest.files.filter((file) =>
					/^namespace\/knowledge\/artifacts\/[^/]+\.json$/.test(file.path),
				).length,
				blobCount: manifest.files.filter((file) =>
					/^namespace\/blobs\/sha256\/[a-f0-9]{2}\/[a-f0-9]{64}$/.test(file.path),
				).length,
