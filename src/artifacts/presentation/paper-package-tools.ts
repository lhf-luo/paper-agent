...
export function latestArtifactSnapshots(manifests: ArtifactManifest[]) {
	const latest = new Map<string, ArtifactManifest["acquisitions"][number]>();
...
export async function hasAvailableArtifact(manifests: ArtifactManifest[]): Promise<boolean> {
	for (const snapshot of latestArtifactSnapshots(manifests)) {
		...
		try {
			...
			if (local.isFile() || local.isDirectory()) return true;
		} catch {
			// A recorded path that no longer exists is not an available artifact.
		}
	}
	return false;
}
...
function artifactLabel(record: PaperRecord, manifests: ArtifactManifest[]): string {
	...
	const acquired = latestArtifactSnapshots(manifests).map((snapshot) =>
			[
	...
	const discovered = [
		...new Map(
			manifests
				...
				.map((candidate) => [candidate.id, `${candidate.id}:${candidate.kind}:${candidate.url}`]),
		).values(),
	];
...
export function buildPaperMaterialPackage(
	...
	versions: PaperVersion[],
	artifactManifests: ArtifactManifest[] = [],
	artifactAvailable = latestArtifactSnapshots(artifactManifests).some(
		(snapshot) => Boolean(snapshot.localPath) && ["cloned", "downloaded", "skipped"].includes(snapshot.status),
	),
): PaperMaterialPackage {
	const missing: string[] = [];
	if (!versions.length && !record.links.some((link) => link.kind === "pdf")) missing.push("pdf");
	if (!artifactAvailable) missing.push("artifact");
	return {
		paperId: record.id,
		record,
		versions,
...
export function registerPaperPackageTools(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"Artifact manifests associated with the paper are loaded from SQLite automatically. Pass artifact_manifest_paths only for unassociated legacy or temporary manifests.",
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			const storedManifests = await store.listArtifactManifests(record.id);
			const pathManifests = await readArtifactManifests(params.artifact_manifest_paths, ctx.cwd);
			const manifests = [
				...new Map([...storedManifests, ...pathManifests].map((item) => [item.pdfSha256, item])).values(),
			];
			const materialPackage = buildPaperMaterialPackage(
				record,
				versions,
				manifests,
				await hasAvailableArtifact(manifests),
			);
			return {
				content: [
					{
