...
import { resolveAdditionalArtifactMetadata } from "./artifact-platform-metadata.ts";
...
export async function resolveArtifactSourceMetadata(
	...
	if (host === "github.com" && parts.length >= 2) {
		...
		return {
			...
			estimatedBytes: typeof value.size === "number" ? value.size * 1024 : undefined,
		};
	}
	...
	return resolveAdditionalArtifactMetadata(candidate, options);
}
