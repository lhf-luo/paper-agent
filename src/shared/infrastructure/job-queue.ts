...
import {
	...
	RetryableJobError,
} from "../domain/background-job.ts";
import { executionPermitOperationId, type PersistedJobRow } from "./job-queue-record.ts";
...
export class PersistentJobQueue {
	...
	async initialize(): Promise<void> {
		...
		this.database.exec(`
			...
			`);
		const columns = this.database.prepare("PRAGMA table_info(background_jobs)").all() as unknown as Array<{
			name: string;
		}>;
		if (!columns.some((column) => column.name === "checkpoint_json")) {
			this.database.exec("ALTER TABLE background_jobs ADD COLUMN checkpoint_json TEXT");
		}
	...
	/**
	 * 一键删除所有终态任务(succeeded/failed/cancelled), 保留仍在运行的(queued/running/paused)。
	 * 返回删除的数量。
	 */
	deleteAll(): number {
		const database = this.requireDatabase();
		...
		try {
			const rows = database
				.prepare("SELECT id FROM background_jobs WHERE status IN ('succeeded','failed','cancelled')")
				.all() as Array<{ id: string }>;
			for (const row of rows) {
				database.prepare("DELETE FROM background_job_operation_claims WHERE job_id = ?").run(row.id);
			}
			const result = database
				.prepare("DELETE FROM background_jobs WHERE status IN ('succeeded','failed','cancelled')")
				.run();
			database.exec("COMMIT");
	...
	private fromRow(row: PersistedJobRow): BackgroundJob {
		return {
			...
			checkpoint: row.checkpoint_json === null ? undefined : (JSON.parse(row.checkpoint_json) as unknown),
		};
	}

	private saveCheckpoint(id: string, value: unknown): void {
		this.requireDatabase()
			.prepare("UPDATE background_jobs SET checkpoint_json = ?, updated_at = ? WHERE id = ?")
	...
	private async run(job: BackgroundJob, handler: BackgroundJobHandler): Promise<void> {
		...
		try {
			const result = await handler(job.input, {
				...
				checkpoint: <T>() => this.get(job.id)?.checkpoint as T | undefined,
				saveCheckpoint: (value) => this.saveCheckpoint(job.id, value),
			});
