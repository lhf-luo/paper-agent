...
- A local Chromium Connector that automatically imports PDFs downloaded by Edge or Chrome into the personal library.
...
External tools do not need to live inside this repository. Put the directories containing their executables in **Settings & diagnostics -> External command directories**, set `externalTools.commandDirectories` in `.paper-agent/config/app.json`, or set `PAPER_AGENT_EXTERNAL_TOOL_PATHS`. Paper Agent checks that override first, then the config, then the inherited system `PATH`.
...
paper-agent models add # optional persistent model key discovery
paper-agent --doctor
paper-agent
```

`paper-agent` opens the local Web workspace on port `43127` by default so the Browser Connector can detect it. The installer creates a user-level command that points to this checkout; rerun `install` if the repository is moved.

To enable browser capture, open `chrome://extensions` or `edge://extensions`, enable developer mode, choose **Load unpacked**, and select this repository's `browser-extension/` directory. While Paper Agent is running, every completed browser PDF download is imported into the current default namespace. After a successful import, the original browser download is removed; failed imports keep it. Downloads made while Paper Agent is stopped are ignored rather than queued.
...
| `paper-agent --agent paper.pdf` | Open a PDF with the paper-research Skill in Pi |
| `paper-agent --agent paper.pdf "Focus on the method"` | Add a natural-language research goal |
...
See the [complete CLI guide](docs/cli.md) for setup, status, verification profiles, and uninstall behavior.
...
9. **Settings & diagnostics** — configure paths, providers, model endpoints, and paste a one-time `pateam1.` team access string without storing secrets in tracked configuration.
...
The bundled `research-wiki` Skill provides an explicit long-term knowledge layer. It discovers the actual tools available in the session, ingests evidence-linked page batches with declaration-level source locations, queries only deposited Wiki pages, and lints Markdown structure, source freshness, links, and indexing. Personal notes remain a separate working repository, and Wiki content is never created automatically after reading a paper.

## Agent chat, Pi terminal, and model configuration

Open **Agent chat** in the normal Web workspace for the primary conversational interface. Configure the provider, model, Base URL, API type, and an ephemeral key there, or run `paper-agent models add` to persist a project model key after discovering available models from the provider's `/models` endpoint. Web-entered keys remain only in service-process memory and must be entered again after a restart; persistent provider keys live in gitignored `.paper-agent/config/auth.json`, separate from model metadata in `models.json`.
...
- API keys and team bearer tokens stay out of git; Web-entered model keys remain only in service-process memory, persistent project model keys live under gitignored `.paper-agent/config/auth.json`, and validated team access lives under gitignored `.paper-agent/team-access.json`;
...
- [Research Wiki](docs/research-wiki.md)
- [Agent tool catalog](docs/agent-tools.md)
- [Team knowledge base](docs/team-knowledge-base.md)
- [Research workflow](docs/research-workflow.md)
- [Standalone team server deployment](team-server/README.md)
