...
- Screen and denoise every externally collected search run before presenting a literature list. This is routine, read-only quality control: perform it proactively and do not ask the user whether routine denoising should be performed.
- When combined results contain off-topic hits, call `filter_search_run_results` on the persisted `search_run_id` instead of re-searching. Derive inclusion terms from the stated research object, problem, and methods. Use exclusion terms only for clearly unrelated concepts; prefer `exclude_scope=title` for broad words so an incidental abstract mention cannot remove a relevant paper.
- Do not universally exclude words such as `optimization`, `structural`, or `topology`. Exclude them only when their usage is demonstrably outside the requested context. Keep uncertain boundary papers as unresolved rather than silently excluding them.
- Filtering never replaces the original search run. Report the original count, retained count, obvious noise categories, and unresolved count. Ask the user only when the inclusion boundary itself is materially ambiguous or changing it would alter the requested research scope.
7. Providers can be circuit-broken mid-collection: when a provider fails on one query (timeout, network error, or rate-limit), the collection stops using that provider for the remaining queries and records a failure for it. Do not treat a circuit-broken provider as "searched successfully" — report its failure and the queries it did cover; other providers continue normally.
...
- `update_literature_sidebar` creates a new list. To correct or extend an existing list, call `edit_literature_sidebar` with that list's `mdUrl` and current revision so the URL and conversation card stay unchanged.
...
4. Use `manage_literature_collections` to list, create, rename, delete, assign, or unassign collections for papers already in the personal library.
5. Use `manage_literature_corpus` with `action=delete` only for an explicitly requested personal-paper deletion and complete its destructive confirmation.
6. Never edit `personal.sqlite`, personal PDF directories, artifact manifests, or search indexes by hand. Use the registered tools and storage APIs.
...
1. Use `download_literature_pdfs` for selected saved papers when PDF bytes are needed. Do not download every unreviewed candidate by default. The tool first uses PDF links already stored with the paper and does not query DOI providers when one succeeds. When no stored PDF exists, DOI-derived candidates are discovered before confirmation. When stored candidates exist, confirmation includes a conditional DOI fallback; Provider lookup starts only after every stored candidate fails. A successful fallback URL is written back to the personal paper so later downloads can reuse it. Provider errors and missing credentials are non-blocking discovery warnings. Only after stored and fallback candidates fail should you report the paper as failed and suggest the paper card's `...` menu and "从本地加载 PDF". Never construct fallback URLs manually or use Bash, curl, or another downloader to bypass this tool's HTTPS, SSRF, redirect, size, and PDF-signature checks.
...
4. Before accepting or generating any GitHub query, call `get_personal_library_paper` and read the stored abstract. Identify the implementation name or project acronym from explicit author language such as "we present X", "our system X", or "the implementation of X". Record the name and its abstract sentence as the query rationale. This lightweight abstract check is mandatory for every paper, even when `discover_paper_artifacts` can generate queries automatically.
5. If the abstract is absent or does not explicitly name the implementation, use `read_pdf` on the abstract and introduction pages (normally physical pages 1-4). Read further only when those pages point to an artifact or named system elsewhere. Do not infer a project name solely from title punctuation, the first title word, an all-caps paper-session label, or a generic domain acronym. Words such as `poster`, `paper`, `machine`, `method`, `system`, `framework`, `analysis`, `detection`, `for`, and a standalone `UAF` are not valid project names without explicit in-paper naming evidence.
6. Run `discover_paper_artifacts` before `acquire_paper_artifacts`, then inspect every reported `github-search` query. Automatic query generation is candidate evidence, not a substitute for the abstract/PDF check. Ignore a generic or unsupported query and do not present its repositories as paper Artifacts.
...
8. Verify that a repository is the paper authors' own release from author-release language, README tool-name/title/method/arXiv/DOI matches, and owner identity. Reject cited baselines, dependencies, tokenizers, paper lists, slide repositories, and repositories merely mentioning or using the paper. A README containing the exact paper title is not by itself high-confidence ownership evidence. GitHub fallback candidates are selected by default only when confidence is high; medium and low matches require explicit `candidate_ids`.
9. Only after deterministic PDF, metadata, tool-name, DOI, author-qualified GitHub searches find no credible candidate may model memory suggest a repository name, owner, or URL. Memory is an untrusted lead, not provenance: verify it against a public GitHub README, author/lab organization page, paper title, DOI, or author evidence. Add only a verified URL through `additional_candidate_urls`; if verification fails, report that no Artifact was found.
10. Complete the exact candidate manifest confirmation before downloading or cloning. For a saved personal paper, pass `paper_id` and namespace so the completed manifest is associated in SQLite.
11. Git repositories are acquired by bounded shallow clone into `<paperId>/artifacts/<project>/`; `fetch_url` is only for read-only primary-source verification. Non-Git files use the bounded HTTP downloader under `<paperId>/artifacts/downloads/`.
12. Use `additional_candidate_urls` for a supported public HTTPS URL found in other public evidence. It is a per-call input, not a config field, and a raw URL must never be passed as `candidate_ids`.
13. Never execute, install dependencies from, or auto-extract acquired content. The default per-artifact limit is 200 MB and the hard maximum is 500 MB; it also applies to the cloned repository working tree and `.git` data.
14. On Windows, do not rename upstream Git paths containing `:` or other NTFS-reserved forms. The acquisition service excludes those paths and conventional committed build/run output through sparse checkout and records the safely named excluded directories in the manifest.
15. Preserve discovery method/query, tool-name evidence, source URL, final URL, SHA-256, readable personal PDF path, commit, license hints, and failure reason when available. SHA-256 is for integrity, exact duplicate detection, and transfer verification; it is not the personal PDF filename. Team storage may still expose a content-addressed blob reference.
