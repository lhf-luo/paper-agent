...
6. Keep the search-run ID, coverage, provider failures, possible duplicates, corpus-hit count, and the bounded candidate Paper ID/title list visible. Retrieve full metadata, abstracts, discovery paths, PDF links, and artifact links from the stored search run only when needed.
...
- When combined results contain off-topic hits, call `filter_search_run_results` on the persisted `search_run_id` instead of re-searching. Use `include_term_groups` when several concepts are required: terms inside a group are alternatives (OR), while every group is required (AND). Use exclusion terms only for clearly unrelated concepts; prefer `exclude_scope=title` for broad words so an incidental abstract mention cannot remove a relevant paper.
...
- Filtering returns every unique matched and unresolved Paper ID/title, with per-group and overall counts, and a `filter_result_id`; deprecated `limit` values do not truncate it. Inspect titles for obvious noise and rerun a stricter deterministic filter if needed. Once the rule is accepted, do not select a subset by title or silently drop unresolved papers. Ask the user only when the inclusion boundary itself is materially ambiguous or changing it would alter the requested research scope.
