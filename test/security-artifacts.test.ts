...
import { extractArtifactCandidates } from "../src/artifacts/application/artifact-discovery.ts";
import { assertPublicUrl, isPrivateAddress } from "../src/shared/infrastructure/network-address.ts";
import {
	fetchPublicUrl,
	fetchWithRetry,
	proxyRequestPath,
} from "../src/shared/infrastructure/network-security.ts";
import { readResponseBody } from "../src/shared/infrastructure/network-content.ts";

describe("network and artifact security", () => {
	it("preserves HTTPS paths and query parameters inside a CONNECT tunnel", () => {
		expect(proxyRequestPath(new URL("https://api.github.com/repos/example/project?ref=main"))).toBe(
			"/repos/example/project?ref=main",
		);
	});
