...
} from "../src/artifacts/application/artifact-content.ts";
import type { ArtifactCandidate, ArtifactManifest } from "../src/literature/domain/literature-types.ts";
...
describe("artifact content and source metadata", () => {
	...
	it("resolves and records a Git ref before publishing a clone", async () => {
		...
		expect(exec.mock.calls.some(([, args]) => args.some((arg) => arg.startsWith("http.curloptResolve=")))).toBe(
			false,
		);
	});
	...
	it("removes a newly cloned repository when it exceeds the artifact byte limit", async () => {
		const root = await mkdtemp(join(tmpdir(), "paper-agent-git-size-"));
		temporaryPaths.push(root);
		const pdfPath = join(root, "paper.pdf");
		await writeFile(pdfPath, "%PDF-fixture");
		const exec = vi.fn(async (_command: string, args: string[]) => {
			if (args.includes("clone")) {
				...
				if (!path) throw new Error("clone path missing");
				await mkdir(join(path, ".git"), { recursive: true });
				await writeFile(join(path, "large.bin"), Buffer.alloc(2_048));
				return { code: 0, stdout: "", stderr: "", killed: false };
			}
			if (args.includes("reset")) return { code: 0, stdout: "HEAD is now ready", stderr: "", killed: false };
			return { code: 1, stdout: "", stderr: "unexpected", killed: false };
		});
		const result = await acquireAuthorized(
			{ exec } as unknown as ExtensionAPI,
			{
				...
				pdfSha256: "c".repeat(64),
				discoveredAt: "2026-01-01T00:00:00.000Z",
				candidates: [candidate("https://github.com/example/oversized", "repository")],
				acquisitions: [],
			},
			{
				maxArtifacts: 1,
				maxBytesPerArtifact: 1_024,
				fetcher: async () => Response.json({ default_branch: "main" }),
				resolver: publicResolver,
			},
		);
		expect(result.manifest.acquisitions.at(-1)).toMatchObject({ status: "failed", bytes: 2_048 });
		expect(result.manifest.acquisitions.at(-1)?.failureReason).toContain("exceeds the 1024 byte artifact limit");
	});
