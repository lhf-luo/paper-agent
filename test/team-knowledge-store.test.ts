...
import type { ArtifactManifest, DerivedRecord } from "../src/literature/domain/literature-types.ts";
import { createTeamKnowledgeService } from "../team-server/src/application/team-knowledge-service.ts";
import { hashTeamTokenValue, TeamTokenRegistry } from "../team-server/src/infrastructure/team-token-registry.ts";
...
describe("team knowledge store", () => {
	...
	it("stores only token hashes and supports online create, rotate, and revoke", async () => {
		...
		const admin = await registry.authenticate("seed-token");
		expect(admin).toMatchObject({ name: "admin", roles: ["admin"] });
		const created = await registry.create(
			{ name: "reader-one", roles: ["reader"], namespaces: ["papers"] },
			admin!.id,
		);
		expect(created.token).toBeTruthy();
		expect(await registry.authenticate(created.token)).toMatchObject({ name: "reader-one", roles: ["reader"] });
		const rotated = await registry.change(
			created.identity.id,
			"rotate",
			{ roles: ["reader", "contributor"], namespaces: ["papers"] },
			admin!.id,
		);
		...
		await registry.change(created.identity.id, "revoke", {}, admin!.id);
