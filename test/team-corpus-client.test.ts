...
describe("team corpus client", () => {
	...
	it("rejects cleartext HTTP for non-loopback hosts before sending a request", async () => {
		const root = await mkdtemp(join(tmpdir(), "paper-agent-team-client-http-"));
		temporaryPaths.push(root);
		await expect(
			saveTeamAccess(root, { serverUrl: "http://example.com/", namespace: "default", token: "secret" }),
		).rejects.toThrow("Team server must be an HTTPS origin (HTTP is allowed only on loopback)");
	});

	it("rejects unsafe namespaces before constructing a request URL", async () => {
		const root = await mkdtemp(join(tmpdir(), "paper-agent-team-client-namespace-"));
		temporaryPaths.push(root);
		await saveTeamAccess(root, { serverUrl: "http://127.0.0.1:1/", namespace: "default", token: "secret" });
		setTeamProjectRoot(root);
