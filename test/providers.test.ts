...
describe("literature providers", () => {
	it("separates keyword-search providers from DOI-only enrichment providers", () => {
		const unpaywall = literatureProviderDefinitions.find((provider) => provider.id === "unpaywall");
		expect(unpaywall?.capabilities).toEqual(expect.arrayContaining(["doi-enrichment", "open-access"]));
		expect(unpaywall?.capabilities).not.toContain("keyword-search");
		expect(() => assertKeywordSearchProviders(["arxiv", "unpaywall"])).toThrow(
			"Providers do not support keyword search: unpaywall",
		);
	});

	it("resolves exact DOI records through Crossref, OpenAlex, and Semantic Scholar", async () => {
		const doi = "10.1000/exact";
		const crossref = await searchCrossrefByDoi(doi, {
			fetcher: async () =>
				Response.json({
					message: {
						DOI: doi,
						...
						link: [{ URL: "https://publisher.example/article/pdf" }],
					},
				}),
		});
		const openalex = await searchOpenAlexByDoi(doi, {
			...
			fetcher: async () =>
				Response.json({
					results: [
						{ id: "https://openalex.org/W1", display_name: "OpenAlex title", doi: `https://doi.org/${doi}` },
					],
				}),
		});
		const semantic = await searchSemanticScholarByDoi(doi, {
			fetcher: async () =>
				Response.json({ paperId: "S1", title: "Semantic title", externalIds: { DOI: doi }, authors: [] }),
		});

		expect(crossref?.identifiers.doi).toBe(doi);
		expect(crossref?.links).toContainEqual({
			url: "https://publisher.example/article/pdf",
			kind: "pdf",
			openAccess: false,
		});
		expect(openalex?.identifiers).toMatchObject({ doi, openAlexId: "https://openalex.org/W1" });
		expect(semantic?.identifiers).toMatchObject({ doi, semanticScholarId: "S1" });
	});
