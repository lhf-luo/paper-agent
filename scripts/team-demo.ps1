...
$accessFile = Join-Path $demoRoot "team-access.json"
...
$serverScript = Join-Path $PaperAgentEnvironment.ProjectRoot "team-server\src\index.ts"
...
function Test-DemoProcess($State) {
	...
	return [bool]($processInfo -and $processInfo.CommandLine -like "*team-server*src*index.ts*")
}
...
$access = @{
	...
	identity = "solo-demo-admin"
}
$access | ConvertTo-Json | Set-Content -LiteralPath $accessFile -Encoding UTF8
$previousAccessFile = $env:PAPER_AGENT_TEAM_ACCESS_FILE
try {
	$env:PAPER_AGENT_TEAM_ACCESS_FILE = $accessFile
	...
	$env:PAPER_AGENT_TEAM_ACCESS_FILE = $previousAccessFile
}
