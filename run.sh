...
install_command() {
	...
	"$NODE_COMMAND" "$AGENT_DIR/scripts/initialize-storage.ts"
...
configure_models() {
	require_dependencies
	exec "$NODE_COMMAND" "$AGENT_DIR/scripts/configure-model.ts" "$@"
}
...
start_agent() {
	...
	while [[ $# -gt 0 ]]; do
		case "$1" in
			--*)
				echo "Unknown agent option: $1. Put the research request after the PDF path." >&2
				exit 1
				;;
			*.[pP][dD][fF])
				pdf="$1"
	...
	local args=("$AGENT_DIR/node_modules/@earendil-works/pi-coding-agent/dist/cli.js" --no-approve --no-extensions --extension "$AGENT_DIR/src/index.ts" --skill "$AGENT_DIR/.agents/skills")
	if [[ -n "$pdf" ]]; then
		if [[ ! -f "$pdf" ]]; then echo "PDF not found: $pdf" >&2; exit 1; fi
		local absolute_pdf
		absolute_pdf="$(cd "$(dirname "$pdf")" && pwd)/$(basename "$pdf")"
		local prompt="/paper \"$absolute_pdf\""
...
team_demo_process_matches() {
	...
	local expected_path="${AGENT_DIR//\\//}/team-server/src/index.ts"
	if [[ "$normalized_command" == *"$expected_path"* ]]; then return 0; fi
	if command -v cygpath >/dev/null 2>&1; then
		local windows_path
		windows_path="$(cygpath -w "$AGENT_DIR/team-server/src/index.ts" 2>/dev/null || true)"
...
team_demo_ensure_server() {
	...
	PAPER_AGENT_TEAM_PORT="$port" \
		nohup "$NODE_COMMAND" "$AGENT_DIR/team-server/src/index.ts" >"$stdout_file" 2>"$stderr_file" </dev/null &
	pid=$!
	local healthy=false
	for _ in {1..30}; do
...
team_demo_command() {
	...
	case "$action" in
		...
		demo)
			...
			local access_file="$TEAM_DEMO_ROOT/team-access.json"
			"$NODE_COMMAND" -e 'const fs=require("node:fs"); fs.writeFileSync(process.argv[1],JSON.stringify({serverUrl:`http://127.0.0.1:${process.argv[2]}`,namespace:"solo-demo",token:fs.readFileSync(process.argv[3],"utf8").trim(),identity:"solo-demo-admin"},null,2)+"\n",{mode:0o600});' "$access_file" "$port" "$TEAM_DEMO_ROOT/token.txt"
			chmod 0600 "$access_file"
			export PAPER_AGENT_TEAM_ACCESS_FILE="$access_file"
...
Paper Agent
  ...
  ./run.sh --agent paper.pdf "Focus on the method and evidence"
