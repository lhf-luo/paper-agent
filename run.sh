...
start_agent() {
	...
	local args=("$AGENT_DIR/node_modules/@earendil-works/pi-coding-agent/dist/cli.js" --no-approve --no-extensions --extension "$AGENT_DIR/src/index.ts" --skill "$AGENT_DIR/skills/literature-corpus-manager" --skill "$AGENT_DIR/skills/literature-survey")
	if [[ -n "$pdf" ]]; then
