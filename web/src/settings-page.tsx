...
import { AccessibleModal, ConsentCard, confirmOperation, LoadingBlock, StatusPill } from "./components";
...
const settingsCategories = [
	...
	{ id: "workspace", label: "界面与存储" },
	{ id: "network", label: "代理配置" },
	{ id: "reader-translation", label: "PDF 选区翻译" },
	{ id: "wiki", label: "知识库与 Obsidian" },
	{ id: "pdf-translation", label: "PDF 翻译引擎" },
	{ id: "mineru", label: "论文解析材料" },
	{ id: "confirmations", label: "操作确认" },
...
export function SettingsPage({ onConfigurationSaved }: SettingsPageProps) {
		...
		try {
			...
			setMessage(
				result.restartRequired
					? "已保存。代理或存储设置有变化，请重启 Paper Agent 使其生效。"
					: "已保存并立即生效。",
			);
			await load();
			await onConfigurationSaved();
	...
	const executePending = async () => {
		...
		try {
			...
			setMessage(
				result.restartRequired
					? "设置已保存。代理或存储设置有变化，请重启 Paper Agent 使其生效。"
	...
	if (!config) return <>{error ? <div className="error-banner">{error}</div> : <LoadingBlock />}</>;
	...
	return (
		<>
			...
			<div className="settings-layout">
				<nav className="panel settings-navigation" aria-label="设置分类">
				...
				<div className="settings-form">
					...
					<div
						...
						hidden={activeCategory !== "workspace"}
					>
					...
					<div
						...
						className="settings-section"
						role="region"
						aria-labelledby="settings-nav-network"
						hidden={activeCategory !== "network"}
					>
						<section className="panel form-panel">
							<span className="eyebrow">NETWORK · 网络连接</span>
							<h2>代理配置</h2>
							<label className="toggle-row">
								<input
									type="checkbox"
									checked={config.network?.proxyEnabled ?? Boolean(config.network?.proxyUrl)}
									onChange={(event) =>
										update((next) => {
											next.network ??= {};
											next.network.proxyEnabled = event.target.checked;
										})
									}
								/>
								<span>启用代理</span>
							...
							<div className="form-grid">
								<label>
									...
									<input
										...
										onChange={(event) =>
											update((next) => {
												next.network ??= {};
												next.network.proxyUrl = event.target.value;
											})
										}
									/>
								</label>
								<label>
									...
									<textarea
										rows={6}
										value={(config.network?.noProxyHosts ?? []).join("\n")}
										onChange={(event) =>
											update((next) => {
												next.network ??= {};
												next.network.noProxyHosts = event.target.value
													.split(/\r?\n/)
													.map((host) => host.trim())
													.filter(Boolean);
											})
							...
							<p className="form-hint">
								代理地址需使用 http:// 或
								https://，并包含端口。直连域名留空时使用内置默认列表。保存代理变更后需重启 Paper Agent。
							</p>
						</section>
					</div>
					<div
						id="settings-panel-reader-translation"
						className="settings-section"
						role="region"
						aria-labelledby="settings-nav-reader-translation"
						hidden={activeCategory !== "reader-translation"}
					>
						<section className="panel form-panel">
					...
					<div
						id="settings-panel-wiki"
						className="settings-section"
						role="region"
						aria-labelledby="settings-nav-wiki"
						hidden={activeCategory !== "wiki"}
					>
						<section className="panel form-panel">
							...
							<div className="form-grid">
								<label className="wide">
									...
									/>
									<small>
										可填写 Obsidian 安装目录或可执行文件路径；Wiki 仓库仍保存在 Paper Agent 数据目录中。
									</small>
								</label>
							</div>
						</section>
					...
					<div
						id="settings-panel-pdf-translation"
						className="settings-section"
						role="region"
						aria-labelledby="settings-nav-pdf-translation"
						hidden={activeCategory !== "pdf-translation"}
					>
						<section className="panel form-panel">
					...
					<div
						id="settings-panel-mineru"
						className="settings-section"
						role="region"
						aria-labelledby="settings-nav-mineru"
						hidden={activeCategory !== "mineru"}
					>
						<section className="panel form-panel">
					...
					<div
						id="settings-panel-confirmations"
						className="settings-section"
						role="region"
						aria-labelledby="settings-nav-confirmations"
						hidden={activeCategory !== "confirmations"}
					>
					...
					<div
						...
						className="settings-section"
						role="region"
						aria-labelledby="settings-nav-search"
						hidden={activeCategory !== "search"}
					>
						<section className="panel form-panel">
					...
					<div
						...
						className="settings-section"
						role="region"
						aria-labelledby="settings-nav-team"
						hidden={activeCategory !== "team"}
					>
									...
									<div className="form-grid two">
										<label>
											...
											<datalist id="team-namespace-options">
												{[
													...new Set(
														[teamAccess.namespace, ...(teamAccess.identity?.namespaces ?? [])].filter(
															Boolean,
														),
													),
												].map((namespace) => (
										...
										<div className="team-access-actions">
											<button
												...
												type="button"
												disabled={
													!teamNamespace.trim() || teamNamespace === teamAccess.namespace || teamBusy
												}
												onClick={() =>
								...
								<>
									...
									<div className="team-access-actions">
										<button
											...
											onClick={() =>
												void prepareTeamAccess({ action: "connect", invite: teamInvite.trim() })
											}
										>
											验证并接入
										</button>
